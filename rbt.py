...
parser.add_argument('--batch', type=int, default=1000, help='')
parser.add_argument('--use_pres', action='store_true', help='Whether to use PRES')
parser.add_argument('--pres_time', action='store_true', help='Whether to use time encoding correction strategy')
...
if __name__ == '__main__':
    ...
    graph_feature=(2*len(df)-len(pd.concat([df['src'],df['dst']]).unique()))/(2*len(df)) # n e
    ...
    def get_staleness_constraint(data, batch_size, graph_feature):
        ...
        time_dic = {}
        for i in np.unique(np.concatenate((data[:,0],data[:,1]))):
            time_dic[i]=data[0][2]
        for i in range(len(range_)):
            ...
            stale = 0
            if i != len(range_)-1:
                now_data = data[range_[i]:range_[i+1]]
                for x in range(len(data[range_[i]:range_[i+1]])):
                    src, dst, time = now_data[x]
                    cache[src]=time
                    cache[dst]=time
                    if src not in dic and dst not in dic:
                        dic[src] = [x]
                        dic[dst] = [x]
                    elif src not in dic and dst in dic:
                        if time-now_data[dic[dst][-1]][2]!=0:
                            stale += (memory_loss() + 2*message_loss()+np.cos((now_data[dic[dst][-1]][2]-time_dic[dst])/(time-time_dic[dst])))*math.log(len(dic[dst]))
                            dic[dst].append(x)
                        dic[src] = [x]
                    elif src in dic and dst not in dic:
                        if time-now_data[dic[src][-1]][2]!=0:
                            stale += (memory_loss() + 2*message_loss()+np.cos((now_data[dic[src][-1]][2]-time_dic[src])/(time-time_dic[src])))*math.log(len(dic[src]))
                            dic[src].append(x)
                        dic[dst] = [x]
                    else:
                        if src==dst:
                            ...
                            if time-now_data[dic[src][-1]][2]!=0 and time-now_data[dic[dst][-1]][2]!=0:
                                stale += (2*memory_loss() + 4*message_loss()+np.cos((now_data[dic[src][-1]][2]-time_dic[src])/(time-time_dic[src]))\
                                    +np.cos((now_data[dic[dst][-1]][2]-time_dic[dst])/(time-time_dic[dst])))*math.log(len(np.unique(dic[src]+dic[dst])))
                                dic[dst].append(x)
                                dic[src].append(x)
                            elif time-now_data[dic[src][-1]][2]!=0:
                                stale += (memory_loss() + 2*message_loss()+np.cos((now_data[dic[src][-1]][2]-time_dic[src])/(time-time_dic[src])))*math.log(len(dic[src]))
                                dic[src].append(x)
                            elif time-now_data[dic[dst][-1]][2]!=0:
                                stale += (memory_loss() + 2*message_loss()+np.cos((now_data[dic[dst][-1]][2]-time_dic[dst])/(time-time_dic[dst])))*math.log(len(dic[dst]))
                                dic[dst].append(x)
        ...
        return np.mean(staleness)+graph_feature*(np.max(staleness)-np.mean(staleness))
    
    def adaptive_split(threshold, data, group_index, batch_size):
        start_list = []
        start = 0
        ...
        while start < end-1:
            ...
            for i in range(start, end):
                ...
                if src not in dic and dst not in dic:
                    ...
                    if time-data[dic[dst][-1]][2]!=0:
                        stale += (memory_loss() + 2*message_loss()+np.cos((data[dic[dst][-1]][2]-time_dic[dst])/(time-time_dic[dst])))*math.log(len(dic[dst]))
                        dic[dst].append(i)
                    dic[src] = [i]
                elif src in dic and dst not in dic:
                    if time-data[dic[src][-1]][2]!=0:
                        stale += (memory_loss() + 2*message_loss()+np.cos((data[dic[src][-1]][2]-time_dic[src])/(time-time_dic[src])))*math.log(len(dic[src]))
                        dic[src].append(i)
                    dic[dst] = [i]
                else:
                    if src==dst:
                        ...
                        if time-data[dic[src][-1]][2]!=0 and time-data[dic[dst][-1]][2]!=0:
                            stale += (2*memory_loss() + 4*message_loss()+np.cos((data[dic[src][-1]][2]-time_dic[src])/(time-time_dic[src]))\
                                +np.cos((data[dic[dst][-1]][2]-time_dic[dst])/(time-time_dic[dst])))*math.log(len(np.unique(dic[src]+dic[dst])))
                            dic[dst].append(i)
                            dic[src].append(i)
                        elif time-data[dic[src][-1]][2]!=0:
                            stale += (memory_loss() + 2*message_loss()+np.cos((data[dic[src][-1]][2]-time_dic[src])/(time-time_dic[src])))*math.log(len(dic[src]))
                            dic[src].append(i)
                        elif time-data[dic[dst][-1]][2]!=0:
                            stale += (memory_loss() + 2*message_loss()+np.cos((data[dic[dst][-1]][2]-time_dic[dst])/(time-time_dic[dst])))*math.log(len(dic[dst]))
                            dic[dst].append(i)
                if stale > threshold or i == end-1:
                    start_list.append(i)
                    start = i
                    for key, value in cache.items():
                        time_dic[key]=value
        ...
        for i in range(len(start_list)):
            if i != len(start_list)-1:
                ...
                batch_size.append(len(range(start_list[i], start_list[i+1])))
        for i in range(len(split)):
            group_index[split[i]] = i
            
        print("average batch size:{:.2f}".format(sum(batch_size)/len(batch_size)))
        return group_index, int(sum(batch_size)/len(batch_size))
    
    train_param['batch_size'] = args.batch
    group_indices = []
    #get train split
    train_index = np.array(df[:train_edge_end].index)
    train_data = np.stack([df[:train_edge_end].src, df[:train_edge_end].dst, df[:train_edge_end].time]).T
    threshold = get_staleness_constraint(train_data, train_param['batch_size'], graph_feature)
    train_group_index, _ = adaptive_split(threshold, train_data, train_index, train_param['batch_size'])
    group_indices.append(np.copy(train_group_index))
    #get val split
    val_index = np.array(df[train_edge_end:val_edge_end].index)
    val_data = np.stack([df[train_edge_end:val_edge_end].src, df[train_edge_end:val_edge_end].dst, df[train_edge_end:val_edge_end].time]).T
    threshold = get_staleness_constraint(val_data, train_param['batch_size'], graph_feature)
    val_group_index, _ = adaptive_split(threshold, val_data, val_index, train_param['batch_size'])
    val_group_index += train_group_index[-1] + 1
    group_indices.append(np.copy(val_group_index))
    #get test split
    test_index = np.array(df[val_edge_end:].index)
    test_data = np.stack([df[val_edge_end:].src, df[val_edge_end:].dst, df[val_edge_end:].time]).T
    threshold = get_staleness_constraint(test_data, train_param['batch_size'], graph_feature)
    test_group_index, _ = adaptive_split(threshold, test_data, test_index, train_param['batch_size'])
    test_group_index += val_group_index[-1] + 1
    group_indices.append(np.copy(test_group_index))
    ...
    for repeat in range(5):
        ...
        if 'combine_neighs' in train_param and train_param['combine_neighs']:
            combine_first = True
        model = GeneralModel(gnn_dim_node, gnn_dim_edge, sample_param, memory_param, gnn_param, train_param, combined=combine_first, use_pres=args.use_pres, num_nodes=g['indptr'].shape[0] - 1).cuda()
        ...
        def eval(group_indices,mode='val'):
            ...
            with torch.no_grad():
                ...
                for _, rows in eval_df.groupby(group_index):
                    ...
                    pred_pos, pred_neg = model(mfgs, pres_time=args.pres_time, neg_samples=neg_samples)
        ...
        for e in range(train_param['epoch']):
            ...
            #sample for all
            t_start = time.time()
            for _, rows in df[:train_edge_end].groupby(group_indices[0]):
                t0 = time.time()
            ...
            for _, rows in df[:train_edge_end].groupby(group_indices[0]):
                ...
                time_prep += t1-t0
                optimizer.zero_grad()
                pred_pos, pred_neg = model(mfgs, time_pres=time_pres, pres_time=args.pres_time)
                loss = creterion(pred_pos, torch.ones_like(pred_pos))
                loss += creterion(pred_neg, torch.zeros_like(pred_neg))
                # if args.use_pres:
                #     loss += 0.1 * (1-inner_prod(model.memory_updater.last_updated_memory, model.memory_updater.prev_memory)).detach()
                ...
                t2 = time.time()
                if mailbox is not None:      
                    mem_edge_feats = edge_feats[rows['Unnamed: 0'].values].cuda() if edge_feats is not None else None
                    root_nodes_gpu = torch.from_numpy(root_nodes).cuda()
                    ts_gpu = torch.from_numpy(ts).cuda()
            ...
            total_time += time_tot
            prep_thread.join()
            print('\ttotal time:{:.2f}s prep time:{:.2f}s sample time:{:.2f}s computation time:{:.2f}s pres time:{:.2f}s'.format(time_tot, time_prep, time_sample, time_tot-time_prep-time_sample, time_pres[0]))
            compute_time.append(time_tot-time_prep-time_sample)
            pres_time.append(time_pres[0])
            ...
            if(early_stop==5):
                print('No improvment over 5 epochs, stop training')
                break
        print('stop at epoch {}'.format(e))
        ...
        repeat_time.append(total_time/(e+1))
        repeat_compute.append(np.mean(compute_time))
        repeat_pres.append(np.mean(pres_time))
        print('repeat {}:'.format(repeat+1))
        if args.eval_neg_samples > 1:
            print('\ttest AP: {:.4f}  test MRR: {:.4f}'.format(ap, auc))
        else:
            print('\ttest AP: {:.4f}  test AUC: {:.4f}  Average time: {:.2f}  compute time: {:.2f}  pres time: {:.2f}\n'.format(ap, auc, total_time/(e+1), np.mean(compute_time), np.mean(pres_time)))
            
    print('average ap: {:.4f} ,average auc: {:.4f} ,average time: {:.2f}  compute time: {:.2f}  pres time: {:.2f}'.format(np.mean(repeat_ap), np.mean(repeat_auc), np.mean(repeat_time), np.mean(repeat_compute), np.mean(repeat_pres)))