...
class MailBox():
    ...
    def prep_input_mails(self, mfg, uni_node, inv_node, uni_edge, inv_edge, use_pinned_buffers=False):
        ...
        for i, b in enumerate(mfg):
            ...
            # b.srcdata['his_mem'] = self.node_memory
...
class GRUMemeoryUpdater(torch.nn.Module):
    ...
    def __init__(self, memory_param, dim_in, dim_hid, dim_time, dim_node_feat, use_pres=False, num_nodes=None, gamma_learnable=False):
        ...
        if self.use_pres:
            ...
            self.memory_dimension = memory_param['dim_out']
            ...
            self.div_term = torch.arange(0, dim_time, step=2, dtype=torch.float).cuda()
            self.div_term = 1./(10000**(self.div_term / dim_time))
                

    def forward(self, mfg, time_pres=None, pres_time=None):
        for b in mfg:
            if self.use_pres:
                # index, _ = torch_scatter.scatter_min(torch.arange(inv_node.shape[0], device=inv_node.device,dtype=inv_node.dtype), inv_node)
                if self.dim_time > 0:
                    time_feat = self.time_enc(b.srcdata['ts'] - b.srcdata['mem_ts'])
                    b.srcdata['mem_input'] = torch.cat([b.srcdata['mem_input'], time_feat], dim=1)
                    # mem_input = torch.cat([b.srcdata['mem_input'][index], time_feat], dim=1)
                updated_memory = self.updater(b.srcdata['mem_input'], b.srcdata['mem'])
                self.prev_memory = b.srcdata['mem']
                if time_pres!=None:
                    time_b = time.time()
                with torch.no_grad():
                    # self.filter.normalization()
                    transition_dense = self.filter.get_incretment(b.srcdata['ID'])
                ...
                memory = (1 - self.gamma) * pred_memory + self.gamma * updated_memory
                with torch.no_grad():
                    change = memory.data.clone() - self.prev_memory.data.clone()
                    if pres_time:
                        delta_time = b.srcdata['ts'] - b.srcdata['mem_ts']
                        num_nodes = delta_time.shape[0]
                        d_time = updated_memory.shape[1]
                        pe = torch.zeros(num_nodes, d_time, device=updated_memory.device)
                        position = delta_time.unsqueeze(-1).float()
                    ...
                    #     change /=change.max() 
                    #     change = 2*change - 1
                    self.filter.update(b.srcdata['ID'], change)
                updated_memory = memory
                if time_pres!=None:
                    time_pres[0] += time.time()-time_b
            else:
                if self.dim_time > 0:
                    time_feat = self.time_enc(b.srcdata['ts'] - b.srcdata['mem_ts'])
                    b.srcdata['mem_input'] = torch.cat([b.srcdata['mem_input'], time_feat], dim=1)
...
class Filter(nn.Module):
    ...
    def normalization(self):
        if not (self.incretment.max().item() == 0):
            ...
            self.incretment /= self.incretment.max()
            self.incretment = 2 * self.incretment -1
