...
if __name__ == '__main__':
    ...
    def get_loss_information():
        a = 0
    
    def get_staleness_constraint(data, batch_size): 
        range_ = np.arange(0,len(data),batch_size) #原始有多少个batch
        if range_[-1] != len(data):
            range_ = np.append(range_, [len(data)]) #多出来的交互单独放一个batch
        ...
        for i in range(len(range_)):
            dic = {}
            stale = 0
            if i != len(range_)-1:  # 不是最后一个
                # origin_nodes = data[range_[i]:range_[i+1],:2].reshape(data[range_[i]:range_[i+1]].shape[0]*2)
                # origin_nodes = np.concatenate((src_index[range_[i]:range_[i+1]],dst_index[range_[i]:range_[i+1]]), axis=0)
                # counter = Counter(origin_nodes[:,0])
                # repeated_nodes = np.array([[row[0], row[1], counter.get(row[0],0)] for row in origin_nodes if counter.get(row[0],0)>1]) # 0列：node_id  1列：time  2列：次数
                for row in data[range_[i]:range_[i+1]]:
                    key1 = row[0]
                    key2 = row[1]
                    value = row[2]
                    # max_time = max(data[range_[i]:range_[i+1],-1])
                    # min_time = min(data[range_[i]:range_[i+1],-1])
                    # bs = range_[i+1]-range_[i]
                    if key1 not in dic:
                        dic[key1] = [value]
                    else:
                        stale += 1/np.exp((value-dic[key1][-1])-1)
                        # for list_i in range(len(dic[key1])-1, -1, -1):
                        #     if list_i == len(dic[key1])-1: # last time
                        #         stale += 1/((value-dic[key1][list_i])-1) # 1-hop neighbor influence
                        #     elif (value-dic[key1][list_i])<((max_time-min_time)/bs): # n-hop 
                        #         alpha = np.power((value-dic[key1][list_i+1])/(value-dic[key1][list_i]),len(dic[key1])-list_i)
                        #         if alpha>0.25:
                        #             stale += alpha/((value-dic[key1][list_i])-1)
                        dic[key1].append(value)
                        
                    if key2 not in dic:
                        dic[key2] = [value]
                    else:
                        stale += 1/np.exp((value-dic[key2][-1])-1)
                        # for list_i in range(len(dic[key2])-1, -1, -1):
                        #     if list_i == len(dic[key2])-1: # last time
                        #         stale += 1/((value-dic[key2][list_i])-1) # 1-hop neighbor influence
                        #     elif (value-dic[key2][list_i])<((max_time-min_time)/bs): # n-hop 
                        #         alpha = np.power((value-dic[key2][list_i+1])/(value-dic[key2][list_i]),len(dic[key2])-list_i)
                        #         if alpha>0.25:
                        #             stale += alpha/((value-dic[key2][list_i])-1)
                        dic[key2].append(value)
    ...
    def adaptive_split(threshold, data, group_index):
        ...
        while start < len(data)-1:  
            ...
            for i in range(start, len(data)):
                ...
                if stale > threshold or i == len(data)-1:
                    end = i
                    break
