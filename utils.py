...
def load_feat(d):
    ...
    if d == 'MOOC':
        edge_feats = torch.randn(411749, 128)
        node_feats = torch.randn(7144, 128)
    if d == 'Flight':
...
def to_dgl_blocks(ret, hist, reverse=False, cuda=True):
    ...
    for r in ret:
        if not reverse:
            ...
            uni_node, inv_node = torch.from_numpy(uni_node), torch.from_numpy(inv_node)
    ...
    mfgs.reverse()
    if not reverse:
        return mfgs, uni_node, inv_node, torch.from_numpy(uni_edge), torch.from_numpy(inv_edge)
    else:
        return mfgs, uni_node, inv_node, uni_edge, inv_edge
