...
class GeneralModel(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param, combined=False, use_pres=False, num_nodes=None):
        ...
        if memory_param['type'] == 'node':
            if memory_param['memory_update'] == 'gru':
                self.memory_updater = GRUMemeoryUpdater(memory_param, 2 * memory_param['dim_out'] + dim_edge, memory_param['dim_out'], memory_param['dim_time'], dim_node, use_pres, num_nodes)
    ...
    def forward(self, mfgs, time_pres=None, pres_time=None, neg_samples=1):
        if self.memory_param['type'] == 'node':
            self.memory_updater(mfgs[0], time_pres, pres_time)
        out = list()
