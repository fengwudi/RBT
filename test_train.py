...
if __name__ == '__main__':
    ...
    def get_staleness_constraint(data, batch_size):
        ...
        for i in range(len(range_)):
            if i != len(range_)-1:
                ...
                nodes = len(np.unique(data[range_[i]:range_[i+1]][:,:2]))
    ...
    def adaptive_split(threshold, data, group_index, batch_size):
        ...
        for i in range(len(threshold)):
            start_list.append(i*batch_size)
        ...
        for i in range(len(start_list)):
            if i != len(start_list)-1:
                split.append(range(int(start_list[i]), int(start_list[i+1])))
    ...
    group_indices = []
    print('batch size:{}'.format(train_param['batch_size']))
    #get train split
    ...
    train_group_index = adaptive_split(threshold, train_data, train_index, train_param['batch_size'])
    ...
    val_group_index = adaptive_split(threshold, val_data, val_index, train_param['batch_size'])
    ...
    test_group_index = adaptive_split(threshold, test_data, test_index, train_param['batch_size'])
    ...
    for repeat in range(1):
        ...
        for e in range(train_param['epoch']):
            ...
            total_time += time_tot
            prep_thread.join()
            print('\ttotal time:{:.2f}s prep time:{:.2f}s sample time:{:.2f}s computation time:{:.2f}s'.format(time_tot, time_prep, time_sample, time_tot-time_prep-time_sample))
            compute_time.append(time_tot-time_prep-time_sample)
            repeat_compute.append(time_tot-time_prep-time_sample)
        ...
        if args.eval_neg_samples > 1:
            ...
            print('\ttest AP: {:.4f}  test AUC: {:.4f}  Average time: {:.2f}  compute time: {:.2f}\n'.format(ap, auc, total_time/(e+1), sum(compute_time)/len(compute_time)))
            
    print('average ap: {:.4f} ,average auc: {:.4f} ,average time: {:.2f}  compute time: {:.2f}'.format(np.mean(repeat_ap), np.mean(repeat_auc), np.mean(repeat_time), np.mean(repeat_compute)))